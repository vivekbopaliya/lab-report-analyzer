      ...
      {trendData.map((trend, index) => (
        <Card key={index}>
          <CardHeader>
            ...
            <CardDescription>Last {trend.data.length} results • Normal range: {trend.normalRange}</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={trend.data}>
                  ...
                  <YAxis />
                  <Tooltip labelFormatter={(value) => new Date(value).toLocaleDateString()} formatter={(value: number) => [`${value} ${trend.unit}`, trend.parameter]} />
